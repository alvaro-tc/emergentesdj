...
class CriterionScoreViewSet(AuditMixin, viewsets.ModelViewSet):
    ...
    def gradesheet(self, request):
        ...
        special_criteria = list(models.CourseSpecialCriterion.objects.filter(course_id=course_id).select_related('parent_criterion').annotate(
            has_tasks=Exists(models.CourseTask.objects.filter(special_criterion=OuterRef('pk')))
        ).order_by('parent_criterion__id', 'id'))
        ...
        # 2. Rows (Students) with search and pagination
        enrollments_qs = models.Enrollment.objects.filter(course_id=course_id).select_related('student').order_by(
            'student__paternal_surname', 'student__maternal_surname', 'student__first_name'
        )
        if search:
        ...
        total_count = enrollments_qs.count()
        offset = (page - 1) * page_size
        enrollments = list(enrollments_qs[offset:offset + page_size])
        enrollment_ids = [e.id for e in enrollments]

        # 3. Scores Data — only for the current page's enrollments
        scores = models.CriterionScore.objects.filter(enrollment_id__in=enrollment_ids)
        special_scores = models.SpecialCriterionScore.objects.filter(enrollment_id__in=enrollment_ids)
        ...
        # 4. Pre-fetch special task data to avoid N+1 queries
        spec_ids_with_tasks = [s.id for s in special_criteria if s.has_tasks]
        special_tasks_by_crit = {}
        special_task_score_map = {}  # (enrollment_id, special_criterion_id) -> {task_id: score}

        if spec_ids_with_tasks and enrollment_ids:
            all_special_tasks = list(models.CourseTask.objects.filter(
                special_criterion_id__in=spec_ids_with_tasks
            ))
            for task in all_special_tasks:
                special_tasks_by_crit.setdefault(task.special_criterion_id, []).append(task)

            all_special_task_scores = models.TaskScore.objects.filter(
                enrollment_id__in=enrollment_ids,
                task__special_criterion_id__in=spec_ids_with_tasks
            ).select_related('task')
            for ts in all_special_task_scores:
                key = (ts.enrollment_id, ts.task.special_criterion_id)
                special_task_score_map.setdefault(key, {})[ts.task_id] = ts.score
            ...
            for struct in structure:
                ...
                for spec in struct['special_criteria']:
                    ...
                    if spec.get('has_tasks'):
                        ...
                        tasks = special_tasks_by_crit.get(actual_id, [])
                        task_scores_for_enr = special_task_score_map.get((enr.id, actual_id), {})
                        total_weight = 0
                        weighted_sum = 0
                        for task in tasks:
                            score_val = task_scores_for_enr.get(task.id, 0)
                            weighted_sum += score_val * task.weight
                            total_weight += task.weight
                        if total_weight > 0:
